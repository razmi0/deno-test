...
type Handler = (req: RequestContext) => Promise<Response> | Response;
...
class Routes {
  ...
  protected addRoute = (
    ...
    this.verbose &&
      console.log(term("green", ` - Added ${method.padEnd(5)} ${targetPath}`));
    return this;
  };
}
...
export default class App extends Routes {
  ...
  public get =
    ((targetPath, handler) =>
      this.addRoute("GET", targetPath, handler)) as RouteHandler;
  public post =
    ((targetPath, handler) =>
      this.addRoute("POST", targetPath, handler)) as RouteHandler;
  public put =
    ((targetPath, handler) =>
      this.addRoute("PUT", targetPath, handler)) as RouteHandler;
  public delete =
    ((targetPath, handler) =>
      this.addRoute("DELETE", targetPath, handler)) as RouteHandler;
  public patch =
    ((targetPath, handler) =>
      this.addRoute("PATCH", targetPath, handler)) as RouteHandler;

  public run(): Response | Promise<Response> {
      ...
      if (match && this.req.method === method) {
        this.verbose &&
          console.log(
            term(
              "blue",
              ` - Matched ${this.req.method} ${targetPath} => ${match.pathname.input}`,
            ),
          );
    ...
    this.verbose &&
      console.log(
        term(
          "red",
          `${this.req.method.padEnd(5)} ${this.req.url} => Not found`,
        ),
      );
